...
def make_masked_state(length, mask_id):
    return [mask_id for _ in range(length)]
...
def initial_state(task, example, harness, length):
    ...
    return make_masked_state(length, harness.mask_id)
...
def exact_leaf_value_enabled(task, config=None):
    config = config or {}
    if "exact_leaf_value" in config:
        return bool(config.get("exact_leaf_value"))
    return bool(getattr(task, "EXACT_LEAF_VALUE", True))


def exact_leaf_candidate_value_enabled(task, config=None):
    config = config or {}
    if "exact_leaf_candidate_value" in config:
        return bool(config.get("exact_leaf_candidate_value"))
    return exact_leaf_value_enabled(task, config)


def state_value_fn(task):
    if hasattr(task, "state_value"):
        return task.state_value
    if hasattr(task, "value_state"):
        return task.value_state
    return None
...
def is_leaf_accept(task, harness, example, state, config=None):
    ...
    if not exact_leaf_value_enabled(task, config):
        return bool((config or {}).get("accept_leaf_without_exact", True))
...
def values_for_states(task, harness, verifier, examples, states, config=None):
    ...
    for idx, state in enumerate(states):
        ...
        if state_value is not None and not is_leaf(state, harness.mask_id):
            values[idx] = clamp_value(state_value(examples[idx], state, harness))
            continue
        if is_leaf(state, harness.mask_id) and exact_leaf_candidate_value_enabled(task, config):
            text = harness.decode_state(state)
            if hasattr(task, "reward_state"):
