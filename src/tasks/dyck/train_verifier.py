...
def train(data_path, output_dir, vocab_size, mask_id, max_length=512, hidden_dim=256, num_layers=4, num_heads=4, dropout=0.25, epochs=10, batch_size=64, global_batch_size=None, eval_batch_size=None, lr=1e-4, seed=2026, device=None):
    ...
    model = TokenVerifier(vocab_size, mask_id, max_length, hidden_dim, num_layers, num_heads, dropout).to(device)
...
def train_from_config(config, device=None):
    ...
    require_known_keys(train_cfg, TRAINING_KEYS, "Dyck verifier training")
    return train(data_path, output_dir, **train_cfg)
