...
class TokenVerifier(nn.Module):
    def __init__(self, vocab_size, mask_id, max_length=512, hidden_dim=256, num_layers=4, num_heads=4, dropout=0.25):
        super().__init__()
        ...
        self.token_emb = nn.Embedding(vocab_size + 1, hidden_dim, padding_idx=mask_id)
...
def load(checkpoint=None, device=None, harness=None):
    del harness
    if not checkpoint:
        raise ValueError("Dyck VGB requires verifier.checkpoint")
