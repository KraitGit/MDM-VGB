...
from utils import barrier, cleanup_distributed, init_distributed, per_rank_batch_size, repo_root, set_seed
...
def _build_model(train_cfg):
    model_config = BertConfig(
        ...
        hidden_size=int(train_cfg.get("dim", 512)),
        intermediate_size=int(train_cfg.get("dim", 512)) * 2,
...
def train_from_config(config, device = None):
    ...
    try:
        ...
        if total_steps <= 0:
            total_steps = max(math.ceil(len(train_rows) / int(train_cfg.get("batch_size", 32))), 1)
