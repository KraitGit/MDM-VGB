...
from utils import (
    ...
    init_distributed,
    per_rank_batch_size,
    read_rollout_payload,
    require_known_keys,
    rollout_rows_from_payload,
    set_seed,
...
def load_snapshot_tensors(data_path, max_length, reward_fn, rank=0, world_size=1, shard_by_rank=True):
    ...
    if isinstance(payload, dict) and payload.get("format") == "qm9_rollout_v1":
        ...
        if input_ids.ndim != 2 or input_ids.shape[1] != int(max_length):
            raise ValueError(f"QM9 verifier expects fixed state length {max_length}")
...
def train_from_config(config, device=None):
    ...
    require_known_keys(train_cfg, TRAINING_KEYS, "QM9 verifier training")
