...
DATA_KEYS = {"cache_dir", "dataset_dir", "split"}
TRAINING_KEYS = {
    "cond_dim",
    "dropout",
    "eval_split_size",
...
def _tokenize(tokenizer, smiles, max_length):
...
def _load_model_and_tokenizer(config, train_cfg, device):
    ...
    model_source = _path(train_cfg["model_source"])
    max_length = int(model_cfg.get("max_length", 32))
    ...
    if bool(train_cfg.get("init_from_scratch", True)):
        ...
        model_config.model_length = max_length
        model_config.hidden_dim = int(train_cfg.get("hidden_dim", 768))
        model_config.cond_dim = int(train_cfg.get("cond_dim", 128))
        model_config.n_blocks = int(train_cfg.get("num_layers", 12))
        model_config.n_heads = int(train_cfg.get("num_heads", 12))
...
def _train_step(model, tokenizer, smiles, optimizer, scheduler, device, use_bf16, max_length):
    x0 = _tokenize(tokenizer, smiles, max_length)["input_ids"].to(device)
...
def train_from_config(config, device=None):
    ...
    require_known_keys(dict(config.get("data", {})), DATA_KEYS, "QM9 base-model data")
    require_known_keys(train_cfg, TRAINING_KEYS, "QM9 base-model training")
    ...
    try:
        ...
        set_seed(seed + rank)
        model, tokenizer, max_length = _load_model_and_tokenizer(config, train_cfg, device_obj)
        ...
        use_bf16 = str(config.get("model", {}).get("dtype", "bf16")) == "bf16" and device_obj.type == "cuda"
        output_dir = _path(train_cfg.get("output_dir", "model_data/qm9/mdlm_qm9"))

        for step in range(1, train_steps + 1):
            batch = random.sample(local_smiles, batch_size)
            loss = _train_step(model, tokenizer, batch, optimizer, scheduler, device_obj, use_bf16, max_length)
