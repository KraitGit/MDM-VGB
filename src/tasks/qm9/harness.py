...
def random_snapshot_steps(batch_size, steps, k, device):
    candidates = list(range(1, max(2, int(steps))))
    rows = []
    for _ in range(int(batch_size)):
        if int(k) <= len(candidates):
            row = random.sample(candidates, int(k))
        else:
            row = random.choices(candidates, k=int(k))
        rows.append(sorted(row))
    return torch.tensor(rows, dtype=torch.long, device=device)


@torch.no_grad()
def sample_mdlm(model, batch_size, seq_len, steps, device, use_bf16, mask_id, snapshots_per_rollout=0):
    xt = torch.full((batch_size, seq_len), mask_id, dtype=torch.long, device=device)
    ...
    if k:
        snapshot_steps = random_snapshot_steps(batch_size, steps, k, device)
        snapshots = torch.empty((batch_size, k, seq_len), dtype=torch.long, device=device)
...
class QM9MdlmHarness:
    def __init__(self, model_name, tokenizer_path, seq_len=32, device=None, dtype="bf16"):
    ...
    def generate_batch(
        ...
        ids = sample_mdlm(
            self.model,
            batch_size,
            int(max_new_tokens or self.seq_len),
            int(steps),
            self.device,
            self.use_bf16,
            self.mask_id,
    ...
    def generate_batch_with_snapshots(self, batch_size, steps=128, snapshots_per_rollout=3, max_new_tokens=None):
        seq_len = int(max_new_tokens or self.seq_len)
        return sample_mdlm(
            self.model,
            int(batch_size),
            seq_len,
...
def load(model_name, tokenizer_path, seq_len=32, device=None, dtype="bf16"):
    return QM9MdlmHarness(model_name=model_name, tokenizer_path=tokenizer_path, seq_len=seq_len, device=device, dtype=dtype)
