    ...
    def values(self, examples, states, harness):
        ...
        for start in range(0, len(states), max(1, self.batch_size)):
            ...
            if ids.ndim != 2 or ids.shape[1] != self.seq_len:
                raise ValueError(f"QM9 verifier expects states with length {self.seq_len}")
            self.stats["state_evals"] += int(ids.shape[0])
...
def load(checkpoint=None, device=None, constant=None, batch_size=8192, **kwargs):
    del kwargs
    if constant is not None:
        return ConstantVerifier(constant)
    if not checkpoint:
        raise ValueError("QM9 VGB requires verifier.checkpoint or verifier.constant")
