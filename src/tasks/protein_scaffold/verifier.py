...
class ProteinStateValueVerifier(nn.Module):
    def __init__(
        ...
        self.batch_size = 256
        self.token_embed = nn.Embedding(int(vocab_size) + 2, hidden_dim)
        self.position_embed = nn.Embedding(int(max_length), hidden_dim)
        self.task_embed = nn.Embedding(self.num_tasks, hidden_dim)
        self.flag_proj = nn.Linear(3, hidden_dim)
