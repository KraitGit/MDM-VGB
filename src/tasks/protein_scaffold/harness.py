...
class SyntheticEvoDiffModel:
    def __init__(self, length):
        self.length = int(length)
        self.vocab = AMINO_ACIDS
        self.vocab_size = len(self.vocab)
        self.mask_id = MASK_ID
        self.backend = "synthetic_evodiff_oadm"

    def logits(self, target, state):
        logits = np.zeros((len(state), self.vocab_size), dtype=np.float32)
        motif = target.get("motif", {})
        for pos in range(len(state)):
            logits[pos] = np.sin(np.arange(self.vocab_size) * 0.37 + pos * 0.11)
            key = str(pos)
            if key in motif:
                logits[pos, int(motif[key])] += 4.0
        return logits
...
def try_evodiff_backend(model_type, length, device=None):
    try:
        from evodiff import pretrained
    except Exception as exc:
        print(json.dumps({"warning": "evodiff import failed, using synthetic backend", "error": str(exc)}), flush=True)
        return None
...
class ProteinScaffoldHarness:
    ...
    def __init__(self, model=None, length=256, device=None):
        self.model = model or SyntheticEvoDiffModel(length)
        self.device = torch.device(device or ("cuda" if torch.cuda.is_available() else "cpu"))
        self.vocab = AMINO_ACIDS
        self.vocab_size = len(self.vocab)
        self.mask_id = MASK_ID
...
def load(device=None, model_type="oa_dm_38M", mock_model=False, length=256, **kwargs):
    del kwargs
    model = None if mock_model else try_evodiff_backend(model_type, length, device=device)
    return ProteinScaffoldHarness(model=model or SyntheticEvoDiffModel(length), length=length, device=device)
