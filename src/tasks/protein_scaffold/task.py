...
from .structure import score_cache_context
...
def cached_score(example, output, cache_dir=None, backend=None, backend_version=None):
    ...
    scoring_cfg = dict(_CONFIG.get("scoring", {}))
    backend = backend or scoring_cfg.get("backend") or example.get("folding_backend") or DEFAULT_BACKEND
    ...
    return read_cache(_cache_dir(example, cache_dir=cache_dir), sequence, backend=backend, backend_version=version, context=context)
...
def score_output(example, output, cache_dir=None, fold=False, model=None, device=None, chunk_size=None, backend=None, backend_version=None):
