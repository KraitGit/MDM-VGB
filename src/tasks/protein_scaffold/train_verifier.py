...
from .verifier import ProteinStateValueVerifier, build_batch, task_key
from utils import per_rank_batch_size, read_rollout, require_known_keys, verifier_training_config
...
def row_target(row, args):
    ...
    if field not in row:
        raise KeyError(f"protein verifier target_field={field!r} is missing from rollout row")
...
def train_from_config(config, device=None):
    ...
    require_known_keys(train_cfg, TRAINING_KEYS, "protein verifier training")
    args = SimpleNamespace(
        data=data_path,
        out_dir=output_dir,
        hidden_dim=train_cfg.get("hidden_dim", 192),
        ...
        pooling=train_cfg.get("pooling", "triple"),
