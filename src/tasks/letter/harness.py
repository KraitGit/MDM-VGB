...
class MaskedLMHarness:
    def __init__(self, model_name=MDLM_QWEN_06B, device=None, dtype="bf16"):
        ...
        self._forward_token_mask = None
        self._forward_token_mask_cache = {}
    ...
    def _invalid_token_ids(self):
        ...
        if self.mask_id is not None:
            invalid.add(int(self.mask_id))
        if self.eos_id is not None:
            invalid.add(int(self.eos_id))
        invalid.update(int(token) for token in (getattr(self.tokenizer, "all_special_ids", []) or []) if token is not None)
        for token, token_id in (getattr(self.tokenizer, "get_added_vocab", lambda: {})() or {}).items():
            text = str(token)
            if text.startswith("<") and text.endswith(">"):
                invalid.add(int(token_id))
        return sorted(token for token in invalid if 0 <= token < int(self.valid_vocab_size))

    def set_forward_token_mask(self, token_mask):
        if token_mask is None:
            self._forward_token_mask = None
            self._forward_token_mask_cache.clear()
            return
        self._forward_token_mask = tuple(bool(item) for item in token_mask)
        self._forward_token_mask_cache.clear()

    def _forward_token_mask_tensor(self, logits):
        if self._forward_token_mask is None:
            return None
        vocab_size = int(logits.shape[-1])
        key = (str(logits.device), vocab_size)
        if key in self._forward_token_mask_cache:
            return self._forward_token_mask_cache[key]
        mask = self._forward_token_mask
        if len(mask) > vocab_size:
            values = mask[:vocab_size]
        elif len(mask) < vocab_size:
            values = mask + (False,) * (vocab_size - len(mask))
        else:
            values = mask
        tensor = torch.tensor(values, dtype=torch.bool, device=logits.device)
        self._forward_token_mask_cache[key] = tensor
        return tensor
    ...
    def _sanitize_logits(self, logits):
        ...
        forward_mask = self._forward_token_mask_tensor(logits)
        if forward_mask is not None:
            logits = logits.masked_fill(~forward_mask.view(*([1] * (logits.dim() - 1)), -1), -float("inf"))
        return logits
