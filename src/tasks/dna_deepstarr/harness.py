...
class D3LMWrapper:
    ...
    def generate(self, prompt="", max_new_tokens=48, temperature=1.0, steps=50, **kwargs):
        del prompt
        remasking = kwargs.pop("remasking", None)
        if remasking is not None:
            kwargs["alg"] = "random" if remasking in (None, "random") else remasking
        allowed = {"top_p", "top_k", "alg", "alg_temp", "return_history"}
