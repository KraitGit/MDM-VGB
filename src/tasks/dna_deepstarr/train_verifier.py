...
def train_from_config(config, device=None):
    ...
    require_known_keys(train_cfg, TRAINING_KEYS, "DNA verifier training")
    ...
    args = SimpleNamespace(
        ...
        mask_id=train_cfg.get("mask_id"),
        pad_id=train_cfg.get("pad_id"),
        max_length=train_cfg.get("max_length", 64),
        hidden_dim=train_cfg.get("hidden_dim", 192),
        num_layers=train_cfg.get("num_layers", 4),
        num_heads=train_cfg.get("num_heads", 6),
        ffn_dim=train_cfg.get("ffn_dim", 768),
